...
helpers.initUnicodeKeyboard = async function (adb) {
  ...
  const appiumIME_UTF7 = 'jp.jun_nama.test.utf7ime/jp.jun.nama.utf7ime.Utf7ImeService';
  logger.debug(`Setting IME to '${appiumIME_UTF7}'`);
  await adb.enableIME(appiumIME_UTF7);
  await adb.setIME(appiumIME_UTF7);
  return defaultIME;
};

helpers.pushSettingsApp = async function (adb) {
  logger.debug("do not install setting app");
  // await adb.install(settingsApkPath, false);
};

helpers.pushUnlock = async function (adb) {
  logger.debug("do not install unlock app");
